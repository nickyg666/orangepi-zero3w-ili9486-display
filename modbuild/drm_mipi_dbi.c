 ...
 * panel resolution, so every flushed rect in framebuffer (virtual)
 * coordinates is downscaled `scale`x in each axis before being written
 * over SPI.  The source framebuffer is assumed to be XRGB8888.
 */
static void mipi_dbi_scaled_fb_dirty(struct iosys_map *src,
				     ...
				     struct drm_rect *rect)
{
	...
	for (j = 0; j < ph; j++) {
		for (i = 0; i < pw; i++) {
			unsigned int x0 = px1 * scale + i * scale;
			unsigned int y0 = py1 * scale + j * scale;
			unsigned int x1 = min_t(unsigned int, x0 + scale, rect->x2);
			unsigned int y1 = min_t(unsigned int, y0 + scale, rect->y2);

			r = g = b = 0;
			n = 0;
			for (sy = y0; sy < y1; sy++) {
				for (sx = x0; sx < x1; sx++) {
					const u8 *q = p + sy * pitch + sx * 4;
					b += q[0];
					g += q[1];
					r += q[2];
					n++;
			      ...
			      struct drm_rect *rect)
{
	...
	if (dbidev->scale > 1 && fb->format->format == DRM_FORMAT_XRGB8888) {
		mipi_dbi_scaled_fb_dirty(src, fb, rect);
		return;
	}
